...
Usage:
    ...
    python -m scripts.dev.players_csv export \
        --json ./data/players.json \
        --csv  ./data/players.csv

    # CSV → players.json（編集後に上書き）
    python -m scripts.dev.players_csv import \
        --csv  ./data/players.csv \
        --json ./data/players.json
"""
...
def main() -> None:
    ...
    p_export.add_argument('--json', type=Path, default=Path('data/players.json'))
    p_export.add_argument('--csv',  type=Path, default=Path('data/players.csv'))

    # import
    p_import = sub.add_parser('import', help='CSV → players.json')
    p_import.add_argument('--csv',  type=Path, default=Path('data/players.csv'))
    p_import.add_argument('--json', type=Path, default=Path('data/players.csv'))
